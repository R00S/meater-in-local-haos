...
// Try to include NimBLE headers
...
#else
  // NimBLE headers not available - provide inline definitions
  #define HAS_NIMBLE_HEADERS 0
  #warning "NimBLE headers not in build path - using inline definitions"
  
  // Inline NimBLE type definitions
  #include <cstdint>
  #include <cstring>
  ...
  extern "C" {
    int ble_hs_id_set_rnd(const uint8_t *rnd_addr) { return 0; }
    int ble_svc_gap_device_name_set(const char *name) { return 0; }
    int ble_gatts_count_cfg(const struct ble_gatt_svc_def *defs) { return 0; }
    int ble_gatts_add_svcs(const struct ble_gatt_svc_def *svcs) { return 0; }
    void ble_gatts_start(void) {}
    int ble_gap_adv_set_fields(const struct ble_hs_adv_fields *adv_fields) { return 0; }
    int ble_gap_adv_start(uint8_t own_addr_type, const void *direct_addr, int32_t duration_ms,
      ...
      ESP_LOGW("meater_ble_server", "BLE advertising not available - NimBLE headers missing");
      return -1;
    }
    int ble_gap_adv_stop(void) { return 0; }
    struct os_mbuf *ble_hs_mbuf_from_flat(const void *buf, uint16_t len) { return nullptr; }
    int ble_gatts_notify_custom(uint16_t conn_handle, uint16_t attr_handle, struct os_mbuf *om) { return 0; }
    int os_mbuf_append(struct os_mbuf *om, const void *data, uint16_t len) { return 0; }
    int ble_hs_mbuf_to_flat(const struct os_mbuf *om, void *flat, uint16_t len, uint16_t *out_copy_len) { return 0; }
  }
#endif
